...
class View {
    ...
    while (true) {
      ...
      switch (action) {
        ...
        case 5:
          return;
        default:
          std::cout << "\033[1;31mWrong command. Try again.\033[0m"
                    << std::endl;
          break;
      }
    }
 ...
 private:
  ...
  void CompareSolutionsTsm() {
    ...
    std::chrono::milliseconds time_sync = MeasureTime(
        [&]() { return RunMultiple(count, TsmSolveMethod::kSync); });
    std::chrono::milliseconds time_async = MeasureTime(
        [&]() { return RunMultiple(count, TsmSolveMethod::kAsync); });

    std::cout << "Times of execution: " << std::endl;
    std::cout << "\t- synchronously:  " << time_sync.count() << "ms"
              << std::endl;
