...
public:
    ...
    Controller(Graph &graph, AntColonySync &ant_colony_sync, AntColonyAsync &ant_colony_async) : graph_(graph), ant_colony_sync_(ant_colony_sync), ant_colony_async_(ant_colony_async) {
    }
    ...
    bool IsEmptyGraph() {
        return graph_.GetSize() == 0;
    }

    TsmResult SolveSyncTsm() {
        return ant_colony_sync_.Solve();
    }

    TsmResult SolveAsyncTsm() {
        return ant_colony_async_.Solve();
    }
